...
void parseFuncSeq(struct nodeFuncSeq* fs) {
	fs->type = 0;
    fs->f = (struct nodeFunction*) calloc(1, sizeof(struct nodeFunction));
    parseFunction(fs->f);
    if (currentToken() != IS) {
		fs->type = 1;
...
void parseFunction(struct nodeFunction* f) {
    expectedToken(PROCEDURE);
    nextToken();
    expectedToken(ID);
	...
	nextToken();
	f->params = calloc(3, sizeof(char*));
	expectedToken(ID);
	f->params[0] = calloc(20, sizeof(char));
	getId(f->params[0]);
	nextToken();
	for (int i=1; i<3; i++) {
		if (currentToken() == COMMA) {
			nextToken();
			expectedToken(ID);
			f->params[i] = calloc(20, sizeof(char));
			getId(f->params[i]);
			nextToken();
		}
	}
...
void parseCall(struct nodeCall* c) {
	...
	expectedToken(LPAREN);
	nextToken();
	c->params = calloc(3, sizeof(char*));
	expectedToken(ID);
	c->params[0] = calloc(20, sizeof(char));
	getId(c->params[0]);
	nextToken();
	for (int i=1; i<3; i++) {
		if (currentToken() == COMMA) {
			nextToken();
			expectedToken(ID);
			c->params[i] = calloc(20, sizeof(char));
			getId(c->params[i]);
			nextToken();
		}
	}
